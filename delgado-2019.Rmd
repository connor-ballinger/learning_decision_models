...
m_surg_cost |> colSums() |> scales::label_dollar()(x = _)
m_surg_cost |> rowSums() |> scales::label_dollar()(x = _)
```
...
-   DOXORUBICINE PEGYLATED is Caelyx.
...
```{r}
# df_drug_chemo <- inner_join(df_drug_costs, df_drug_doses, by = "drug") |> 
#   select(drug, )
# df_drug_chemo
...
m_costed_drugs
unique(my_drugs$drug)
```

# Not sure how costs work

-   Do we assume all patients receive treatment before the model begins? No mortality or anything? i.e. Initial start-up costs like surgery. 

-   Maybe model starts immediately after surgery, so pre-surgical chemo is guaranteed, as is first cycle of post-surgical chemo. Subsequent cycles are not guaranteed.

-   Per paper: 

    -   Neoadjuvant receive 3 cycles of paclitaxel with carboplatin prior to surgery and 3 cycles post-surgery.
    -   Adjuvant administered post-surgery in Stable state.
    -   Post-progression chemo administered post-progression for Stages I and II.
    -   Advanced treatment for Stage III post-progression and both states for Stage IV. But they haven't define advanced chemotherapy...

<br><br>
-   Are the calculated costs only for a single cycle (and treatment requires 6)?
...
```{r}
# 3 cycles of palitaxel and carboplatin

m_total_cost_neo <- m_costed_drugs[c("Carboplatin", "Paclitaxel"), 3:4] %*% 
  # diag(a_trace["Stable", c("III", "IV") , "0"]) %*% ######################### per person cost, trace later
  diag(m_params[grep("neoadjuvant", dimnames(m_params)[[1]]), 3:4]) * 
  3

dimnames(m_total_cost_neo)[[2]] <- c("III", "IV")
m_total_cost_neo

# Assume they get another 3 regardless of state? Or only those in Stable?
...
-   Do I want track how much has been spent on each drug? Yes (and tracking drug spending as a category would be a bare minimum). How? An array with the following dimensions: stage, cycle, (from state, to state, )... Maybe this array doesn't need everything, it can use the transition matrix and the trace for calculations... Separate arrays for each cost category rather than separate dimensions?

```{r}
m_params[v_names_tmt_strategy, ]
...
m_cost_adjuvant <- 
  ...
  m_costed_drugs %*% 
  diag(m_params[grep("_adjuvant", dimnames(m_params)[[1]]), ])
...
-   Bring the per person cost here, multiply trace for total cost at each cycle, for each stage.
...
a_costs <- array(data = 0, 
                 dim = c(n_cycles + 1, 
                         4,
                         length(rownames(m_costed_drugs))), 
                 dimnames = list("cycle" = 0:n_cycles,
                                 "stage" = v_names_stages,
                                 "drug_cost" = rownames(m_costed_drugs)))

# Neoadjuvant

## three cycles before surgery

a_costs["0", colnames(m_total_cost_neo), rownames(m_total_cost_neo)] <- 
  a_costs["0", colnames(m_total_cost_neo), rownames(m_total_cost_neo)] +
  m_total_cost_neo %*% diag(a_trace["Stable", colnames(m_total_cost_neo) , "0"])

## what about three cycles post-surgery?
...
a_costs["0", , ] <- 
  a_costs["0", , ] +
  t(m_cost_adjuvant %*% diag(a_trace["Stable", , "0"]))

# second cycle, received in second period

a_costs["1", , ] <- 
  a_costs["1", , ] +
  t(m_cost_adjuvant %*% diag(a_trace["Stable", , "1"]))

a_costs["0", , ] |> format(big.mark = ",", scientific = 999)
...
m_drug_total_cost <- m_line1_use[rownames(m_costed_drugs),] * m_costed_drugs * 6 

m_drug_total_cost <- m_drug_total_cost %*% diag(v_init_stages) # note use of diag to create matrix
