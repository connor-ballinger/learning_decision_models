...
df_drug_costs <- df_drug_costs |> 
  mutate(quant = if_else(!is.na(quantity_ml), quantity_ml, quantity_mg))
...
-   Are the calculated costs only for a single cycle (and treatment requires 6)? How do you keep track of what cycle they should be up to without tunnel states? How do you track lines of treatment?
...
a_costs["0", colnames(m_cost_neo_pp_3), rownames(m_cost_neo_pp_3)] <- 
  a_costs["0", colnames(m_cost_neo_pp_3), rownames(m_cost_neo_pp_3)] +
  m_cost_neo_pp_3 %*% diag(a_trace["Stable", colnames(m_cost_neo_pp_3) , "0"])
...
a_costs["0", , ] |> conr::round_sensibly() |> format(big.mark = ",", scientific = 999)
