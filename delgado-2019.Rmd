...
-   Not sure. Maybe it doesn't differ at all from the metrics for cycles because this isn't a true semi-Markov. Not the case for Prog state.
...
# what's this?
# df_trace_semi |>
#   filter(State == "Progressed") |>
#   mutate(Volume = Sojourn * N) |>
#   summarise(Sum = sum(Volume), .by = Stage) |>
#   mutate(Dead = rowSums(a_trace_semi["Dead", , "173", ], na.rm = TRUE),
#          Ave = Sum / Dead)


# df_trace_semi |> 
#   filter(State == "Progressed") |> 
#   summarise(mean = round(mean(Sojourn)), q = list(quantile(Sojourn)), .by = Stage) |> 
#   unnest_wider(col = q)
