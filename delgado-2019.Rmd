...
head(which(is.na(a_prob_semi), arr.ind = TRUE))
a_prob_semi[, , "I", 170:174, 170:174, drop = FALSE] |>
  aperm(perm = c("from", "to", "stage", "sojourn", "cycle")) |>
  round(3)
```

Maybe name dimensions of arrays when subsetting?
  ...
  for (t in 2:(n_cycles + 1)) { # indexing by integer, not cycle name. 
    for (sojourn in 1:(n_cycles + 1)) { 
      ...
      if (sojourn == 1) {
        ...
        diag(move) <- 0 # dealt with remainers below
...
# Probs sum to 1
## Recall probs haven't varied by sojourn in Delgado, so can fix sojourn
tracker <- 0
...
-   Not sure there is anything to see...
...
-   See above: slight discrepancy for Carboplatin cost for Stage IV for cycles 1, 2 (and maybe more). Can't see why... Bevacizumab is fine.
