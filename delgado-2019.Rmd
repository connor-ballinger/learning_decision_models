...
(v_drug_costs <- deframe(df_drug_costs[, c("drug", "cost")]))
...
fn_calc_drugs <- function(drug_name, stage = "stage_i") {
  ...
  df_drug = df_drug_costs[df_drug_costs$drug == drug_name, ]
  ...
  tibble(drug = df_drug$drug,
         info = df_drug$unit,
         doses = as.vector(opt$constraints)[1:4],
         costs = opt$objective,
         dose_needed = dose,
         stage = stage,
