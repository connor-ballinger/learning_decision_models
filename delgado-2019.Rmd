...
-   **23/10 Try incorporating subsequent lines of treatment by tracking length of time in progressed state. Will have to use some kind of blanket rule - every x months, all patients in progressed state receive next treatment.** Using exhaustive tunnel states for Progressed state resulted in a probability array which was more than 20 times the size (w.r.t. memory) of the sojourn array.
...
# patients who progressed in first cycle get 2nd line treatment

# a_costs[, , 2] <- a_costs[, , 2] +
  m_costed_drugs * 
  m_drug_use[, "line2"] %*% 
  t(rowSums(a_trace_semi["Progressed", , 2, ], na.rm = TRUE))
