...
-   Note that age-specific probabilities of death (incorporating OC and background mortality) are not equal to P(OC_death) + P(bgr_death). **Need to add the two rates together before converting to a probability**. (Note that there is no added mortality risk for stable state)
    ...
    -   Add 2 rates and then calculate probability, or calculate probabilities and add? Delgado uses the latter.

# To do

-   Need to do background mortality properly (place-holder used).

-   Optionally, use population figures instead of existing probability (would require adding yearly cohort with pop growth too). 

-   Definitely add costs.
...
v_rate_bgr_mort <- as.vector(life_table$rate_bgr_mort, mode = "double")
names(v_rate_bgr_mort) <- as.vector(life_table$age, mode = "character")
# str(v_rate_bgr_mort)  
# v_rate_bgr_mort[["56"]]

# make array with three dimensions? cycle, bgr_mort, stage?
...
df_prob <- df_bgr_mort |> 
  left_join(df_rates, by = "stage") |> # [, c("stage", "rate_os")]
...
v_names_stages |> 
  walk(\(x) a_prob["Stable", "Progressed", x, ] <<- df_prob_stages[[x]]$prob_s_p)
v_names_stages |> 
  walk(\(x) a_prob["Stable", "Dead", x, ] <<-  df_prob_stages[[x]]$prob_s_d)
v_names_stages |> 
  walk(\(x) a_prob["Progressed", "Dead", x, ] <<- df_prob_stages[[x]]$prob_p_d)

v_names_stages |> 
  walk(\(x) a_prob["Stable", "Stable", x, ] <<- 1 - a_prob["Stable", "Progressed", x, ] - a_prob["Stable", "Dead", x, ])
v_names_stages |> 
  walk(\(x) a_prob["Progressed", "Progressed", x, ] <<- 1 - a_prob["Progressed", "Dead", x, ])
...
for (stage in v_names_stages) {
  for (t in 2:n_cycles) { # indexing by integer when cycle names is a string. Could use dimnames but I need t-1...
