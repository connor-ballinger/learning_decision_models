...
-   Analysis of sojourn time.

-   **Currency conversion from Euros.**
...
-   See Hawkins 2005: semi-Markov relies on a 3-D transition matrix (from state, to state, t) where t is time spent in current state. 

-   Use `drop = FALSE` when printing arrays for more explicit labelling.

-   Using exhaustive tunnel states for Progressed state resulted in a probability array which was more than 20 times the size (w.r.t. memory) of the sojourn array.
...
-   When to implement rounding? Nonsensical for fractions of people in trace at each time point, but would equate to a lot of rounding errors.

-   Bit unsure about censoring at times. 

      -   Median is not affected by censoring - either the median person has experienced the event or they have been censored.
...
-   5-year PFS and OS were published by stage - Figures 9 and 24. Delgado extrapolated assuming an exponential distribution.

$$S(t) = e^{-\lambda t}$$

-   Can solve for $\lambda$ when $S$ and $t$ are known (i.e. proportion event-free at five years).

-   For example, if time to median survival was known:
...
-   Number of patients in each state over 10-year horizon, by FIGO stage.
...
-   Normalise Stages for more explicit comparison.
...
-   Mean PFS and OS, and aggregate share of time without experiencing an event. I don't have to worry about censoring so long as results are reported for 10-year period, which aligns with Delgado (i.e. I have restricted mean survival time).

```{r}
a_trace_norm <- a_trace

for (cycle in dimnames(a_trace_norm)[["cycle"]]) {
  a_trace_norm[, , cycle] <- a_trace_norm[, , cycle] %*% diag(1 / v_init_stages)
}

rowSums(a_trace_norm["Stable", , ]) * n_cycle_length

colSums(a_trace_norm[c("Stable", "Progressed"), , ]) |> 
  rowSums() *
  n_cycle_length
```

-   Portion of time in PFS and OS.

```{r}
rowSums(a_trace_norm["Stable", , ]) * 1 / n_cycles
...
-   Stable represents PFS and Alive represents OS.

```{r}

# function to retrieve PFS and OS info by combining S and P into Alive
fn_alive <- function(df, states_existing, state_new, keep = TRUE) {
  
  if (keep) {
    
    df |> 
      mutate(State_2 = case_when(State %in% states_existing ~ state_new,
                                 .default = NA)) |> 
      pivot_longer(cols = contains("State"), values_to = "State") |> 
      select(-c(name)) |> 
      reframe(Portion = sum(Portion, na.rm = TRUE), .by = c(Cycle, Stage, State, Year)) |> 
      drop_na(State) |> 
      mutate(State = factor(State, c("Stable", "Progressed", state_new, "Dead"), ordered = TRUE)) |> 
      arrange(Cycle, Stage, State)
    ...
    df |> 
      mutate(
        State = fct_recode(
          State, 
          set_names(states_existing, rep(state_new, length(states_existing)))
        )
      ) |> 
      reframe(Portion = sum(Portion, na.rm = TRUE), 
              .by = c(Cycle, Stage, State, Year))
    
  }
}

df_graph <- df_trace_normalised |> 
  fn_alive(c("Stable", "Progressed"), "Alive")

ggplot(data = df_graph) +
  geom_line(aes(x = Year, y = Portion, colour = Stage)) + 
  facet_wrap(vars(State), ncol = 4)

ggplot(data = filter(df_graph, State %in% c("Stable", "Alive"))) +
  geom_line(aes(x = Year, y = Portion, colour = State)) + 
  facet_wrap(vars(Stage), ncol = 4) +
  labs(colour = "") +
  scale_color_discrete(labels = c("Stable" = "PFS", "Alive" = "OS"))
```
...
# Logically, sojourn must not be greater than current cycle
dims_a_prob_semi <- dimnames(a_prob_semi)

for (cycle in dims_a_prob_semi[["cycle"]]) {
  for (sojourn in dims_a_prob_semi[["sojourn"]]) {
    
    if (as.integer(sojourn) > as.integer(cycle)) {
      a_prob_semi[, , , cycle, sojourn] <- NA
    }
    
  }
}
rm(cycle, sojourn)
# head(which(is.na(a_prob_semi[, , "I", , ]), arr.ind = TRUE), n = 10)

a_prob_semi[, , "I", 1:3, 1:3, drop = FALSE] |>
  aperm(perm = c("from", "to", "stage", "sojourn", "cycle")) |>
  round(3)
...
v_init_stages <- c(1155, 195, 1116, 681) # Delgado figures
names(v_init_stages) <- v_names_stages
v_init_stages_norm <- v_init_stages * 1 / v_init_stages
v_init_states <- c(1, 0, 0) # all patients start in stable state
...
# dims_a_trace_semi <- dimnames(a_trace_semi)
# would be messy indexing by names as I frequently use +1 / -1.

for (stage in v_names_stages) {
  for (t in 2:(n_cycles + 1)) { # indexing by integer, not cycle name. 
    for (sojourn in 1:(n_cycles + 1)) { 
      
      if (sojourn == 1) {
        
        # for those changing state
        
        state_prev <- rowSums(a_trace_semi[, stage, t-1, ], na.rm = TRUE) # past sojourn irrelevant
        move <- a_prob_semi[, , stage, t, sojourn] # probs don't vary by sojourn
        diag(move) <- 0 # dealt with remainers in else if
        
        a_trace_semi[, stage, t, sojourn] <- 
          state_prev %*% move
        
      } else if (between(sojourn, 2, t)) {
        
        # for those staying in state
        
        state_prev <- a_trace_semi[, stage, t-1, sojourn-1]
        remain <- diag(a_prob_semi[, , stage, t, sojourn])
        ...
        a_trace_semi[, stage, t, sojourn] <- NA
        
      }
    }
  }
}
rm(stage, t, sojourn, state_prev, move, remain)

a_trace_semi[, , 1:3, 1:3, drop = FALSE] |> 
  aperm(perm = c("state", "stage", "sojourn", "cycle"))
```

-   A simpler look, using Stage I only:

```{r}
a_trace_semi[, "I", 1:3, 1:3, drop = FALSE] |> 
  aperm(perm = c("state", "sojourn", "cycle", "stage"))
```

-   Tunnels approach is a much slower alternative.
...
```{r}
# No illegal probs
sum(a_prob_semi >= 0 & a_prob_semi <= 1, na.rm = TRUE) - length(which(!is.na(a_prob_semi)))

# Probs sum to 1
## Recall probs haven't varied by sojourn in Delgado, so can fix sojourn, otherwise iterate over sojourn
tracker <- 0

for (stage in v_names_stages) {
  for (t in 1:n_cycles+1) {
    tracker = tracker + rowSums(a_prob_semi[, , stage, t, 1]) - 1
  }
}
...
-   How would I calculate stats around sojourn time? Also, note that censoring occurs.

```{r}
df_trace_semi <- as.data.frame(aperm(a_trace_semi)) |> 
  mutate(Sojourn = 0:n_cycles) |> 
  pivot_longer(cols = -(Sojourn), 
               names_to = c("Cycle", "Stage", "State"),
               names_sep = "\\.", 
               values_drop_na = TRUE,
               values_to = "N") |> 
  mutate(Cycle = as.integer(Cycle),
         ...
         State = factor(State, 
                        levels = c("Stable", "Progressed", "Dead"), 
                        ordered = TRUE)) |> 
  filter(N != 0) # I think this is fine
```

-   Can replicate results for standard Markov:

```{r}
a_trace_semi_norm <- a_trace_semi

for (cycle in dimnames(a_trace_semi_norm)[["cycle"]]) {
  for (sojourn in dimnames(a_trace_semi_norm)[["sojourn"]]) {
    
    a_trace_semi_norm[, , cycle, sojourn] <- 
      a_trace_semi_norm[, , cycle, sojourn] %*%
      diag(1 / v_init_stages)
    
  }
}

rowSums(a_trace_semi_norm["Stable", , , ], na.rm = TRUE, dims = 1) *
  n_cycle_length

colSums(a_trace_semi_norm[c("Stable", "Progressed"), , , ], na.rm = TRUE) |> 
...
### Analyse Sojourn Time

-   Aggregate time in Progressed state followed by proportion of time in Progressed state.

```{r}
a_trace_semi_norm[c("Progressed"), , , , drop = FALSE] |> 
  aperm(c("state", "stage", "sojourn", "cycle")) |> 
  rowSums(na.rm = TRUE, dims = 2) * 
  n_cycle_length
...
-   Stage I does not reach median for OS or PFS.

```{r}
# function to calculate survival / sojourn time for a particular quantile

fn_calc_time <- function(array, state, type, denom, quantile) {
  v_names_stages |> 
    set_names() |> 
    map_chr(\(x) names(
      which.min(
...
-   For standard Markov, time until median proportion is reached in Stable and Dead states (median in Progressed is never reached as state membership is temporary). 

    -   Censoring has not been accounted for here (right?).

```{r}
fn_calc_time(a_trace_semi_norm, "Stable", "cycle", v_init_stages_norm, 0.5)

fn_calc_time(a_trace_semi_norm, "Dead", "cycle", v_init_stages_norm, 0.5)

```

### Sojourn Distribution in Progressed State

-   Conditional on dying? Yes, which accounts for censoring.

-   Note median time in Progressed state for Stage IV is five cycles. A single line of CT lasts six cycles. 90% of these patients do not have time to receive three lines of treatment even if they were given consecutively. Even the other stages do not have much time to receive CT after first progression.

```{r}
v_dead_portion <- rowSums(a_trace_semi_norm["Dead", , n_cycles + 1, ], na.rm = TRUE)

# In which cycle was the given quantile reached for sojourn in Progressed state.
c(0, 0.01, 0.1, 0.5, 0.9, 0.99, 1) |> 
  set_names() |> 
  map(\(x) fn_calc_time(a_trace_semi_norm, "Progressed", "sojourn", v_dead_portion, x)) |> 
  list_transpose() |> 
  as.data.frame.list() |> 
...
-   Cumulative distribution along sojourn at the last cycle.

```{r}
df_trace_semi_norm <- as.data.frame(aperm(a_trace_semi_norm)) |> 
  mutate(Sojourn = 0:n_cycles) |> 
  pivot_longer(cols = -(Sojourn), 
               names_to = c("Cycle", "Stage", "State"),
               names_sep = "\\.", 
  ...
  mutate(Cycle = as.integer(Cycle),
         ...
         State = factor(State, 
                        ...
                        ordered = TRUE)) |> 
  filter(Portion != 0) |> 
  mutate(CumulativeSojourn = cumsum(Portion), .by = c(State, Stage, Cycle))


ggplot(data = filter(df_trace_semi_norm, Cycle == 173, State != "Stable"),
       aes(x = Sojourn, y = CumulativeSojourn, colour = Stage)) +
  geom_line() +
  facet_wrap(vars(State))

```

-   Entries into states (i.e. Sojourn = 0).

```{r}
ggplot(data = filter(df_trace_semi_norm, Cycle > 0, Sojourn == 0)) +
  geom_line(aes(x = Cycle, y = Portion, colour = Stage)) +
  facet_wrap(vars(State))
```
...
df_service_costs <- df_service_costs |> 
  ...
  mutate(service = make_clean_names(service),
         cost = as.double(cost))
df_service_costs[df_service_costs$service == "day_hospital_price_per_minute",]$cost <- 0.32
df_service_costs
...
-   Often have to add costs to existing costs, not reassign the existing matrix value...
...
# Sojourn is a similar case to State.
...
a_costs[rownames(m_cost_neo_pp), colnames(m_cost_neo_pp), "0"] <- 
  a_costs[rownames(m_cost_neo_pp), colnames(m_cost_neo_pp), "0"] +
  (m_cost_neo_pp %*% 
     diag(a_trace_semi["Stable", colnames(m_cost_neo_pp), "0", "0"])) * 3
...
1:3 |> 
  walk(\(x) a_costs[rownames(m_cost_neo_pp), colnames(m_cost_neo_pp), x] <<- 
         ...
         m_cost_neo_pp %*% 
         diag(rowSums(a_trace_semi["Stable", colnames(m_cost_neo_pp), x, ], na.rm = TRUE))
  )
...
1:6 |> 
  walk(\(x) a_costs[, , x] <<- 
         a_costs[, , x] +
         m_cost_adjuvant %*% 
         diag(rowSums(a_trace_semi["Stable", , x, ], na.rm = TRUE))
  )
...
a_costs[1:2, , 1:2, drop = FALSE]
```

-   **See above**: large discrepancy for Carboplatin cost for Stage IV for cycles 1, 2 (and maybe more). Can't see why... Bevacizumab is fine.
...
-   Median OS is never more than 1.3 years greater than median PFS - not much time to get through another 6 lines of treatment, at least in half of cases.

-   **Solution**: use sojourn to choose when to assign cost of CT.

-   **Question**: reasonable to terminate first-line CT due to progression and immediately implement second-line CT?

-   Can I track how many receive CT? Particularly, track completion rates of each line.

    -   99% of IV have sojourn time less than 35, 90% under 18 cycles. Just do all lines back-to-back.

```{r}
# more generally, for sojourn = 0:5 in Prog receives 2nd line, regardless of
# cycle but I need to track cycle for discounting etc.

fn_prog_lines <- function(line, cycle, sojourn){
  line = paste0("line", line)
  m_costed_drugs *
    m_drug_use[, line] %*% 
    t(a_trace_semi["Progressed", , cycle, sojourn])
}

fn_add_cost <- function(line, cycle, sojourn) {
  a_costs[, , cycle] <<- 
    a_costs[, , cycle] +
    fn_prog_lines(line = line, cycle = cycle, sojourn = sojourn)
}

fn_all_lines_cost <- function(line) {
  lower = ((line - 2) * 6) + 1
  upper = lower + 6
  for (cycle in dimnames(a_costs)[["cycle"]]) {
    for (sojourn in lower:upper) {
      if (sojourn <= as.integer(cycle)) {
        fn_add_cost(line, cycle, sojourn) 
      }
    }
  }
}

2:7 |> # lines of therapy, for all stages, cycles
  walk(\(x) fn_all_lines_cost(x))
```
...
df_drug_costs |> filter(drug %in% df_drug_pall$drug)


```

-   Palliative care is given to the patients in their last 48 days of life.

-   Up to 93.3% of the patients receive follow-up care at outpatient hospitals, while the remaining (6.7%) are assisted by palliative home care team.

-   Patients need a mean of 9.5 home visits of palliative-care services

-   those who receive follow-up at the outpatient hospital are seen by a nurse. 

-   In both cases, patients also pay four visits, on average, to the primary-care doctor 

-   they spent their terminal phase of their illness at home (59.6%) or at the hospital (40.4%).

-   The last 3 days of this terminal phase, patients stay at home, and they are visited twice a day by a nurse. 

-   Of these, 14% receive sedation.

```{r}
df_service_costs |> 
  filter(service %in% c("primary_care_home_care",
                        "nursing_care_home_care",
                        "palliative_care_hospitalization",
                        "palliative_care_services_home_care",
                        "home_care_price_per_hour")) |> 
  mutate(portion = c())
```
