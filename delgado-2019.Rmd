...
-   When to implement rounding? Non-sensical for fractions of people in trace at each time point, but would equate to a lot of rounding errors.
...
-   Is this mean PFS and mean OS? If so, it would be making an error on censoring - not sure how to manage that manually...

    -Yes, mean PFS and OS, and aggregate share of time without experiencing an event. I don't have to worry about censoring so long as results are reported for 10-year period, which aligns with Delgado (i.e. I have restricted mean survival time).
...
-   Is this the portion of time in PFS and OS? Does censoring need to be accounted for?
...
-   Not sure. Maybe it doesn't differ at all from the metrics for cycles because this isn't a true semi-Markov. 

    -   Note: Not the case for Prog state, which is what I'm interested in. Time in Stable can be analysed from standard Markov.
