...
-   How to add calculations for 2nd to 7th lines? No tunnel states, so no way of knowing which line is applicable. Same issue exists within lines as patients could change states between cycles.
      ...
      -   See Cheng 2021: they assume everyone in Progressed state is receiving treatment.
...
# str_extract(df_drug_costs$unit, "([0-9]*\\.?[0-9]+) MG/ML", group = 1)
```
...
-   How do you keep track of what cycle they should be up to without tunnel states? 
...
a_costs <- array(data = 0, 
                 ...
                 dimnames = list("cycle" = 0:n_cycles, # the order is switched from other arrays...bad?
                                 "stage" = v_names_stages,
...
a_costs["0", colnames(m_cost_neo_pp), rownames(m_cost_neo_pp)] <- 
  a_costs["0", colnames(m_cost_neo_pp), rownames(m_cost_neo_pp)] +
...
1:3 |> 
  walk(\(x) a_costs[x, colnames(m_cost_neo_pp), rownames(m_cost_neo_pp)] <<- 
         a_costs[x, colnames(m_cost_neo_pp), rownames(m_cost_neo_pp)] +
...
1:6 |> 
  walk(\(x) a_costs[x, , ] <<- 
         a_costs[x, , ] +
         t(m_cost_adjuvant %*% diag(a_trace["Stable", , x]))
  )

a_costs["0", , ] |> 
  conr::round_sensibly() |> 
  format(big.mark = ",", scientific = 999)
