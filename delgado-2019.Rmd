...
```{r}
# find the cycle where the value is 0.5
v_names_stages |> 
  set_names() |> 
  map(\(x) 
      unname(
      ...
      unname(
        which(
          round(
            rowSums(
              colSums(
                a_trace_semi_norm[c("Stable", "Progressed"), x, , ],
                na.rm = TRUE
              ),
              na.rm = TRUE
            ),
...
v_names_stages |> 
  ...
  map(\(x) 
      names(
        which.min(
          abs(
            colSums( # Adding cycles together
              a_trace_semi_norm["Progressed", x, , ],
              na.rm = TRUE
            ) - (v_dead_portion[[x]] / 2) # half of all who died
          )
        )
      ) 
  )
#colSums(a_trace_semi_norm["Progressed", "IV", , ], na.rm = TRUE)
```
...
```{r}
ggplot(data = filter(df_trace_semi, Sojourn == Cycle)) +
  geom_line(aes(x = Cycle, y = N, colour = Stage)) +
  facet_wrap(vars(State))


ggplot(data = filter(df_trace_semi, Cycle > 0, Sojourn == 0)) +
  geom_line(aes(x = Cycle, y = N, colour = Stage)) +
  facet_wrap(vars(State))
```
...
```{r}
# more generally, for sojourn = 0:5 in Prog receives 2nd line, regardless of cycle
# but I need to track cycle for discounting etc.
...
for (cycle in dimnames(a_costs)[["cycle"]]) {
  for (sojourn in 1:6) {
    if (sojourn <= as.integer(cycle)) {
      fn_add_cost("line2", cycle, sojourn) 
    }
  }
}
rm(cycle, sojourn)

# How long before patients receive 3rd line of treatment? First, need to
# understand how long they stay in Prog state.
```
