...
for (cycle in 0:n_cycles + 1) {
  for (sojourn in 0:(n_cycles + 1)) { # struggling with indexing for some reason. Behaving as char?
    if (sojourn > cycle) {
...
a_prob_semi[, , "I", 1:2, 1:2] |> aperm(perm = c("from", "to", "sojourn", "cycle"))

# head(which(is.na(a_prob_semi), arr.ind = TRUE), n = 10)
...
a_trace_semi[, "I", 1:5, 1:5] |> aperm(perm = c("state", "sojourn", "cycle"))
 
a_trace_semi[, , 174, ] |> rowSums(na.rm = TRUE, dims = 2L)
